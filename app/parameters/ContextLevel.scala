...
class ParagraphBreakIterator extends BreakIterator {
  ...
  var breakIndices: Array[Int] = null
  var currentB: Int = 0
...
object ContextLevel extends Enum[ContextLevel] {
  case object CHARACTER extends ContextLevel {
    def apply(expand: Int): BreakIterator = if (expand>0) return new ExpandingBreakIterator(BreakIterator.getCharacterInstance,expand) else BreakIterator.getCharacterInstance
  }
  case object WORD extends ContextLevel {
    def apply(expand: Int): BreakIterator =  if (expand>0) return new ExpandingBreakIterator(BreakIterator.getWordInstance,expand*2) else BreakIterator.getWordInstance
  }
  case object SENTENCE extends ContextLevel {
    def apply(expand: Int): BreakIterator =  if (expand>0) return new ExpandingBreakIterator(BreakIterator.getSentenceInstance,expand) else BreakIterator.getSentenceInstance
  }
  case object PARAGRAPH extends ContextLevel {
    def apply(expand: Int): BreakIterator =  if (expand>0) return new ExpandingBreakIterator(new ParagraphBreakIterator(),expand) else new ParagraphBreakIterator()
  }

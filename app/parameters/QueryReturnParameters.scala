...
case class QueryReturnParameters()(implicit request: Request[AnyContent], ia: IndexAccess) {
  ...
  def toJson() = Json.obj("limit"->limit,"contextLevel"->contextLevel.entryName,"contextExpand"->contextExpand,"smoothing"->smoothing,"sumScaling"->sumScalingString,"fields"->(termVectorFields++sortedDocValuesFields++storedSingularFields++storedMultiFields++numericDocValuesFields),"returnMatches"->returnMatches,"returnNorms"->returnNorms)
}

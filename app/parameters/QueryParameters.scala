...
case class QueryParameters(prefix: String = "", suffix: String = "")(implicit request: Request[AnyContent], ia: IndexAccess) {
  ...
  val query: Option[String] = p.get(prefix+"query"+suffix).map(v => if (!v(0).startsWith("<")) "<"+ia.indexMetadata.levels.last.id+"|"+v(0)+"|"+ia.indexMetadata.levels.last.id+">" else v(0))

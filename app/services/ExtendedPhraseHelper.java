...
public class ExtendedPhraseHelper extends PhraseHelper {
     ...
     */
    public ExtendedPhraseHelper(Query query, String field, Predicate<String> fieldMatcher, Function<SpanQuery, Boolean> rewriteQueryPred,
                        Function<Query, Collection<Query>> preExtractRewriteFunction,
                        boolean ignoreQueriesNeedingRewrite, boolean matchFullSpans) {
        ...
        new WeightedSpanTermExtractor(field) {
            ...
            protected void extractWeightedTerms(Map<String, WeightedSpanTerm> terms, Query query, float boost)
                    throws IOException {
                query.createWeight(ExtendedUnifiedHighlighter.EMPTY_INDEXSEARCHER, ScoreMode.COMPLETE_NO_SCORES, boost)
    ...
    public void createOffsetsEnumsForSpans(LeafReader leafReader, int docId, List<OffsetsEnum> results) throws IOException {
        ...
        while (spansPriorityQueue.size() > 0) {
            ...
            if (matchFullSpans) {
                PhraseOffsetSpanCollector tspanCollector = new PhraseOffsetSpanCollector();
                spans.collect(tspanCollector);
                SpanCollectedOffsetsEnum starts = tspanCollector.termToOffsetsEnums.get(tspanCollector.terms.get(0));
                ...
                SpanCollectedOffsetsEnum offsetsEnum = spanCollector.termToOffsetsEnums.get(span);
                if (offsetsEnum == null) {
    ...
    private class PhraseOffsetSpanCollector implements SpanCollector {

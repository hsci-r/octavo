...
class TermVectorDiffController @Inject() (implicit iap: IndexAccessProvider, env: Environment, conf: Configuration) extends AQueuingController(env, conf) {
  ...
  def termVectorDiff(index: String) = Action { implicit request =>
    ...
    val grouper = p.get("grouper").map(_.head).map(apScript => {
      ...
      b.setProperty("ia", ia)
      b.setProperty("gp", gp)
      b.setProperty("tvq1", tvq1)
      b.setProperty("tvq2", tvq2)
      b.setProperty("tvpl", tvpl)
      b.setProperty("tvpa", tvpa)
    ...
    val qm = Json.obj("grouper"->p.get("grouper").map(_.head),"attrTransformer"->p.get("attrTransformer").map(_.head),"attrs"->attrs,"attrLengths"->attrLengths,"meaningfulTerms"->meaningfulTerms) ++ gp.toJson ++ tvq1.toJson ++ tvq2.toJson ++ tvpl.toJson ++ tvpa.toJson
    getOrCreateResult("termVectorDiff",ia.indexMetadata, qm, gp.force, gp.pretty, () => {
      ...
      val tvm1f = Future { getGroupedAggregateContextVectorsForQuery(ia.indexMetadata.levelMap(qlevel1),searcher(qlevel1, SumScaling.ABSOLUTE), termVector1Query,tvpl,extractContentTermsFromQuery(termVector1Query),grouper,attrs,attrLengths,attrTransformer,tvpa,gp.maxDocs/2) }
      val tvm2f = Future { getGroupedAggregateContextVectorsForQuery(ia.indexMetadata.levelMap(qlevel2),searcher(qlevel2, SumScaling.ABSOLUTE), termVector2Query,tvpl,extractContentTermsFromQuery(termVector2Query),grouper,attrs,attrLengths,attrTransformer,tvpa,gp.maxDocs/2) }
      val tvm1 = Await.result(tvm1f, Duration.Inf)
      val tvm2 = Await.result(tvm2f, Duration.Inf)
      val obj = (tvm1.keySet ++ tvm2.keySet).map(key => {
        var map = Json.obj("attrs"->Json.toJson(attrs.zip(key).toMap),

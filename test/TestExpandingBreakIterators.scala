...
class TestExpandingBreakIterators {
    ...
    def testExpandingBreakIterator: Unit = {
        ...
        bi = ContextLevel.TOKEN(2,2)
        bi.setText(str)
        assertEquals(" @theresa_may, ",str.substring(Math.max(bi.preceding(16),0),bi.following(16)))
        bi = ContextLevel.TOKEN(3,3)
        bi.setText(str)
        assertEquals("theresa_may @theresa_may, today",str.substring(Math.max(bi.preceding(16),0),bi.following(16)))
        bi = ContextLevel.TOKEN(1,1)
        bi.setText(str)
        assertEquals("@theresa_may,",str.substring(bi.preceding(16),bi.following(16)))
    }
